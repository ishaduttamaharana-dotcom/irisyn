...
### Phase 2 — Data & Telemetry
- **Priority**: Critical
- **Objective**: Real hardware telemetry collector from developer host computer, live WebSockets streaming, persistent historical metrics DB storage, telemetry freshness SLA calculation.
- **Exit Criteria**: Real host computer values collected; UI updates live via WebSockets; historical time-series persisted.

### Phase 3 — Digital Twin Core
- **Priority**: Critical
- **Objective**: Persistent Digital Twin state engine, physical-to-digital state synchronization, operating modes (`NORMAL`, `HIGH_LOAD`, `DEGRADATION`, `FAULT`), multi-factor health degradation models.
- **Exit Criteria**: Asset has persistent digital twin state; twin state responds to telemetry anomalies.
